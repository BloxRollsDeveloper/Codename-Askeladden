...
public class BabyCreeperAnimationController : MonoBehaviour
{
    ...
    public void UpdateAnimation(bool spawn, bool explode, bool takeDamage, bool death)
    {
        var nextAnimation = GetAnimation(spawn,explode, takeDamage, death);
    ...
    private int GetAnimation(bool isSpawning, bool isExploding, bool takeDamage, bool isDying)
    {
        ...
        if (isSpawning) return LockAnimation(Spawn, spawnAnimTime);
        if (isDying) return LockAnimation(Death, deathAnimTime);
        if (takeDamage) return LockAnimation(Damage, damageAnimTime);
        if (isExploding) return LockAnimation(Explode, explodeAnimTime);
